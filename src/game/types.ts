...
export interface Game {
  ...
  /* --- то, что дописывает физика поверх движка (см. `physics.ts`) --- */

  /**
   * Ход подвески, м. Ноль — машина стоит ровно на пружинах; плюс — кузов
   * подкинуло. Движок это поле не трогает: без физики оно так и остаётся нулём,
   * и камера просто не качается по вертикали.
   */
  heave: number;
  /**
   * Продольный клевок кузова, рад. Плюс — нос вниз (торможение и удар), минус —
   * присел на корму (разгон). Тоже не трогается движком.
   */
  pitchBody: number;
  /**
   * Крен кузова от физики, рад. Отдельно от `roll`, который считает движок:
   * если писать физический крен в то же поле, движок на следующем шаге начнёт
   * демпфировать уже испорченное значение, и получится петля с накоплением.
   */
  rollBody: number;
  /**
   * Насколько задняя ось «поплыла» вбок, 0…1. Нужна и картинке, и звуку: это
   * ровно тот занос, на котором визжат шины.
   */
  skid: number;
  ...
  /* --- служебное --- */
  /**
   * Остаток нерасходованного времени между кадрами, с. Симуляция идёт
   * фиксированным шагом, а кадры приходят с частотой монитора — здесь копится
   * разница. Внутреннее поле движка, снаружи трогать нечего.
   */
  acc: number;
