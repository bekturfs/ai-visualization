...
/* ---------- ручки, которые крутят на глаз ---------- */

/**
 * В отличие от всего остального в этом файле — **изменяемый** объект.
 *
 * Сюда вынесено то, что нельзя вычислить, а можно только подобрать глазами на
 * настоящем экране: яркость разметки, усиление огней, амплитуда мерцания.
 * Однажды это уже правилось вслепую по описанию «горит слишком ярко», и вышло
 * два лишних круга; панель в dev крутит эти числа прямо в игре, а найденные
 * значения возвращаются сюда как новые умолчания.
 *
 * Кто читает эти значения — читает их при сборке своей системы, поэтому панель
 * после правки пересобирает сцену. Живьём меняется только мерцание.
 */
export const TUNE = {
  road: {
    /** Осевая: непрозрачность вблизи и вдали. */
    centerNear: 0.66,
    centerFar: 0.5,
    /** Краевые линии. */
    sideNear: 0.4,
  ...
  lights: {
    /** Головы фонарей. */
    glow: 1.1,
    /** Пятно света на асфальте. */
    pool: 0.75,
    /** Огни городка на склоне. */
    town: 0.8,
  },
  sky: {
    /** Амплитуда мерцания звёзд и её вариант для «меньше движения». */
    twinkle: 0.38,
    twinkleCalm: 0.16,
  },
};
