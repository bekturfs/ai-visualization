...
const STEPS: StoryStep[] = [
  {
    ...
    text: (
      <>
        ...
        повторяют. Кстати, «ИИ», «нейросеть» и «модель» здесь — одно и то же:
        программа-угадывалка, настроенная на примерах. Внизу — конвейер этого
        трюка, и он сейчас работает по-настоящему: смотри, как рождается слово.
      </>
    ),
  },
  {
    ...
    <Layout crumb="00 · Общая картина" crumbEn="the big picture">
      ...
      <div className="grid grid-cols-[1.05fr_.95fr] items-start gap-4 max-lg:grid-cols-1">
          ...
          <Card title="Текст (растёт на глазах)">
            ...
            {deadEnd && (
              <motion.div
                ...
                className="mt-2.5 rounded-lg border border-[#7a5226] bg-[#3d2a16]/60 px-3 py-2 text-[13.5px] text-amber"
              >
                🛑 Это не поломка! Наша кроха-модель «закончила мысль»: такого
                сочетания слов она в своих 50 предложениях не видела, и ставок
                на продолжение у неё нет. У настоящих больших моделей опыт в
                миллиарды раз больше, поэтому они так не замолкают.{" "}
                <button
                  onClick={() => reset()}
                  className="ml-1 cursor-pointer rounded-md border border-[#7a5226] bg-[#4a3219] px-2 py-0.5 text-amber hover:border-amber"
                >
                  ⟳ начать заново
                </button>
              </motion.div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <Card title="🤔 Погоди, это же просто T9 — откуда тогда «ум»?">
            <p className="my-1 text-[13.5px] leading-relaxed text-soft">
              Хитрость в том, что{" "}
              <b className="text-ink">хорошо угадывать слова — очень трудно</b>.
              Чтобы угадать слово в «столица Франции — …», нужно знать факты. В
              «она положила ключи в свою …» — грамматику. В «значит, x = …» —
              уметь считать. Модель, которую заставили триллионы раз угадывать
              слова во всём интернете, волей-неволей выучивает язык, факты и
              логику — не потому, что её этому учили отдельно, а потому, что{" "}
              <b className="text-ink">иначе слово не угадать</b>. Из одного
              простого правила на гигантском масштабе вырастает сложное
              поведение.
            </p>
          </Card>
